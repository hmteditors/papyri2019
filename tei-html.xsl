...
<xsl:stylesheet version="1.0" xmlns:cts="http://chs.harvard.edu/xmlns/cts3" xmlns:dc="http://purl.org/dc/elements/1.1" xmlns:tei="http://www.tei-c.org/ns/1.0"
	...
	<xsl:template match="tei:gap">
		<!-- Toggle between the two lines below depending on whether you want to show supplied text or not -->
		<!--<span class="suppliedText">&lt;<xsl:apply-templates/>&gt;</span>-->
		<span class="suppliedText"><xsl:call-template name="replaceGap"/></span>
	</xsl:template>
	<!-- begin replacing supplied text with non-breaking spaces -->
	<xsl:template name="replaceGap">
		<xsl:variable name="currentChar">1</xsl:variable>
		<xsl:variable name="stringLength">
			<xsl:value-of select="@extent"/>
		</xsl:variable>
		<xsl:variable name="myString"> _ </xsl:variable>
		<xsl:call-template name="replaceGapRecurse">
			<xsl:with-param name="currentChar">
				<xsl:value-of select="$currentChar"/>
			</xsl:with-param>
			<xsl:with-param name="stringLength">
	...
	<xsl:template name="replaceGapRecurse">
		<xsl:param name="currentChar"/>
		<xsl:param name="myString"/>
		<xsl:param name="stringLength"/>
		<xsl:choose>
			<xsl:when test="$currentChar &lt;= $stringLength">
				<xsl:call-template name="replaceGapRecurse">
					<xsl:with-param name="currentChar">
						<xsl:value-of select="$currentChar + 1"/>
					</xsl:with-param>
					<xsl:with-param name="stringLength">
						<xsl:value-of select="$stringLength"/>
					</xsl:with-param>
					<xsl:with-param name="myString">
						<xsl:value-of select="concat($myString,' _ ')"/>
					</xsl:with-param>
				</xsl:call-template>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template match="tei:seg[@type='word']">
		<span class="wordSeg"><xsl:apply-templates/></span>
	</xsl:template>
	<xsl:template match="tei:seg">
		<span class="notWordSeg"><xsl:apply-templates/></span>
	</xsl:template>
	<!--<xsl:template match="tei:gap">
		<span class="gap">&#160;&#160;&#160;</span>
	</xsl:template>-->
